...
- **verbose=True**：默认开启，测试执行时会打印详细信息
- **游戏结束**：状态为 WIN 或 FLOW_DRAW
- **有分数数据**：`context.final_scores` 已计算

### 显示内容

1. **最终得分**：每个玩家的得分（正数绿色，负数红色，零无色）
2. **获胜者**：和牌的玩家
...
如果不需要分数显示，可以设置 `verbose=False`：

```python
result = ScenarioBuilder("测试").run(verbose=False)
```

### 牌格式控制

可以使用 `tile_format` 参数控制牌的显示格式：

```python
# 使用牌名（默认）
result = ScenarioBuilder("测试").run(tile_format="name")  # 一万、五条等

# 使用数字ID
result = ScenarioBuilder("测试").run(tile_format="number")  # 0、5等
```
