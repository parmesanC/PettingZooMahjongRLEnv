...
from src.drl.buffer import MixedBuffer, RolloutBuffer
from src.drl.mappo import MAPPO, SupervisedLearning
from src.drl.config import NFSPConfig, MAPPOConfig
...
class NFSP:
    ...
    def __init__(self, config, device: str = "cuda"):
        ...
        # [P1-5] 创建 CentralizedCriticNetwork 并传递给 MAPPO
        ...
        # 创建 centralized buffer（用于 centralized critic 训练）
        from .buffer import CentralizedRolloutBuffer

        self.centralized_buffer = CentralizedRolloutBuffer(
            capacity=config.nfsp.rl_buffer_size
        )

        # 创建训练器（传递 centralized_critic）
        self.mappo = MAPPO(
            ...
            device=device,
            centralized_critic=self.centralized_critic,  # [P1-5] 传递 centralized_critic
        )
    ...
    def store_transition(
        ...
        if is_best_response:
            self.buffer.add_sl_experience(obs, action_mask, action_type, action_param)
    ...
    def train_step(self, training_phase: int = 1, centralized_buffer=None) -> Dict:
        ...
        Args:
            ...
            centralized_buffer: CentralizedRolloutBuffer 实例（可选，用于 Phase 1-2）
        ...
        # Phase 1-2: 使用 centralized critic 训练
        if (
            training_phase in [1, 2]
            and self.mappo.centralized_critic is not None
            and centralized_buffer is not None
        ):
            # 使用 centralized buffer 和 centralized critic
            if len(centralized_buffer) > 0:
                centralized_stats = self.mappo.update_centralized(
                    centralized_buffer, training_phase=training_phase
                )
            ...
            if len(self.buffer.rl_buffer) >= self.config.nfsp.rl_batch_size:
                rl_stats = self.mappo.update(
                    self.buffer.rl_buffer, training_phase=training_phase
                )
        ...
        if len(self.buffer.sl_buffer) >= self.config.nfsp.sl_batch_size:
            sl_stats = self.sl_trainer.update(
                self.buffer.sl_buffer, batch_size=self.config.nfsp.sl_batch_size
            )
            ...
            if final_obs is not None:
                with torch.no_grad():
                    ...
                    if final_action_mask is not None:
                        mask_tensor = (
                            torch.FloatTensor(final_action_mask)
                            .unsqueeze(0)
                            .to(self.device)
                        )
                    else:
    ...
    def _prepare_obs(self, obs: Dict[str, np.ndarray]) -> Dict[str, torch.Tensor]:
        """
        将观测转换为张量
        完全匹配 Wuhan7P4LObservationBuilder 的输出格式
        ...
        for key, value in obs.items():
            if isinstance(value, dict):
                ...
                tensor_value = torch.FloatTensor(value).to(self.device)
                # 确保所有观测都是至少1维的
                if tensor_value.dim() == 0:
                    tensor_value = tensor_value.unsqueeze(0)
                # 添加批次维度
                tensor_obs[key] = tensor_value.unsqueeze(0)
        return tensor_obs
